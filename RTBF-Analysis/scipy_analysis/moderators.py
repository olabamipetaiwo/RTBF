"""
Scipy-only reimplementation of src/moderators.py (step 9). Data prep (scale
scoring, N_FLOOR, willingness mapping) reused from src.stats / src.willingness;
the test layer is reimplemented: kruskal (already scipy) + scipy_analysis.dunn
post-hoc (was scikit-posthocs), chi2_contingency/fisher_exact (already scipy,
via scipy_analysis.verify), Holm via scipy_analysis.correction (was statsmodels).
"""
...
def posthoc_for_sig(df, group_col, outcome_series, kept):
    ...
    dunn = dunn_test(sub, val_col="v", group_col="g")
