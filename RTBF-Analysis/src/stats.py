"""
RTBF statistical tests (steps 1-6).

BETWEEN-METHOD (independent groups; Q4.2/Q5.2 single-select)
  1. Q: Do deletion methods differ in perceived protection / effort / benefit-loss?
     Kruskal-Wallis omnibus, per scale x scenario, methods n>=N_FLOOR
  2. Q: If so, WHICH method pairs differ?
     Dunn's post-hoc + Holm, only where omnibus significant
BETWEEN-SCENARIO (within-subject; paired base n=176)
  3. Q: Are the less-vs-more difference scores normally distributed?
     Shapiro-Wilk on difference scores -> normal? parametric : non-parametric
  4. Q: Does data sensitivity change perceived protection / effort / benefit-loss
        for the SAME person using the SAME methods?
     Paired t-test OR Wilcoxon signed-rank, per scale, Holm across the 3
  5. Q: Does data sensitivity change what users EXPECT deletion to do, or whether
        they VERIFY it?
     McNemar per option on expectation (Q4.4/Q5.4) & verification (Q4.5/Q5.5)
METHOD-SWITCHING
  6. Q: When data is more sensitive, do users switch to different (heavier) methods?
     Stuart-Maxwell marginal homogeneity + transition matrix (Q4.2 -> Q5.2)

Effect sizes reported throughout. Nulls = underpowered, NOT "no difference".
Kept methods are n~14-83: tests detect only LARGE effects.
"""

import itertools
import numpy as np
import pandas as pd
from scipy import stats
from statsmodels.stats.multitest import multipletests
from statsmodels.stats.contingency_tables import mcnemar, SquareTable
import scikit_posthocs as sp
import pingouin as pg
from src.screen import get_bases
from src.reporting import save_report

LIKERT = {"Strongly Disagree": 1, "Stongly Disagree": 1, "Disagree": 2,
          "Neutral": 3, "Agree": 4, "Strongly Agree": 5}
SCALES = {"protection": (".7", 5), "effort": (".8", 4), "benefit_loss": (".9", 3)}
SCEN = {"less": ("Q4", "Q4.2"), "more": ("Q5", "Q5.2")}
N_FLOOR = 14


def scale_score(df, base, q, k):
    items = [f"{base}{q}_{i}" for i in range(1, k + 1)]
    return df[items].apply(lambda c: c.map(LIKERT)).mean(axis=1)


def eps2(H, n, k):
    return round((H - k + 1) / (n - k), 3) if n - k > 0 else np.nan


def rank_biserial_indep(a, b):
    U, _ = stats.mannwhitneyu(a, b, alternative="two-sided")
    return round(1 - (2 * U) / (len(a) * len(b)), 3)


# ---------- 1 & 2 : between-method ----------
def between_method(bases, lines):
    lines.append("\n" + "=" * 70)
    lines.append("STEPS 1-2  BETWEEN-METHOD (Kruskal-Wallis -> Dunn's + Holm)")
    lines.append("=" * 70)
    omnibus_pvals = []   # [(label, p_raw)] — omnibus KW only, for global correction
    for scen, (base, mcol) in SCEN.items():
        df = bases[scen].copy()
        df = df[df[mcol] != "Other (Please specify)"]
        counts = df[mcol].value_counts()
        kept = counts[counts >= N_FLOOR].index.tolist()
        for scale, (q, k) in SCALES.items():
            df["_s"] = scale_score(df, base, q, k)
            groups = {m: df.loc[df[mcol] == m, "_s"].dropna().values for m in kept}
            H, p = stats.kruskal(*groups.values())
            n_tot = sum(len(v) for v in groups.values())
            sig = p < 0.05
            omnibus_pvals.append((f"[{scen}|{scale}] method KW", p))
            lines.append(f"\n[{scen} | {scale}] KW  H={H:.2f}  p={p:.4f}  "
                          f"eps2={eps2(H, n_tot, len(kept))}  -> {'SIG' if sig else 'ns'}")
            if not sig:
                continue
            # Dunn's post-hoc (pooled ranks), Holm-corrected
            long = df[df[mcol].isin(kept)][[mcol, "_s"]].dropna()
            dunn = sp.posthoc_dunn(long, val_col="_s", group_col=mcol, p_adjust="holm")
            for m1, m2 in itertools.combinations(kept, 2):
                pv = dunn.loc[m1, m2]
                if pv < 0.05:
                    r = rank_biserial_indep(groups[m1], groups[m2])
                    lines.append(f"    * {m1[:28]:28} vs {m2[:28]:28} "
                                  f"p_holm={pv:.4f}  r={r}")
    return omnibus_pvals


# ---------- 3 & 4 : between-scenario, Likert ----------
def between_scenario_likert(paired, lines):
    lines.append("\n" + "=" * 70)
    lines.append("STEPS 3-4  BETWEEN-SCENARIO Likert (Shapiro -> t/Wilcoxon, Holm x3)")
    lines.append("=" * 70)
    pvals, results = [], []
    for scale, (q, k) in SCALES.items():
        less = scale_score(paired, "Q4", q, k)
        more = scale_score(paired, "Q5", q, k)
        d = (more - less).dropna()
        _, p_norm = stats.shapiro(d)
        normal = p_norm > 0.05
        if normal:
            t, p = stats.ttest_rel(more.loc[d.index], less.loc[d.index])
            dz = round(d.mean() / d.std(ddof=1), 3)          # Cohen's dz
            test, stat, es = "paired-t", round(t, 3), f"dz={dz}"
        else:
            r = pg.wilcoxon(more.loc[d.index], less.loc[d.index])
            p = float(r["p_val"].iloc[0])
            rb = round(float(r["RBC"].iloc[0]), 3)   # matched rank-biserial (correct)
            test, stat, es = "Wilcoxon", round(float(r["W_val"].iloc[0]), 1), f"r={rb}"
        pvals.append(p)
        results.append((scale, test, normal, stat, es, d.mean()))
    holm = multipletests(pvals, method="holm")[1]
    for (scale, test, normal, stat, es, mdiff), p_adj, p_raw in zip(results, holm, pvals):
        arrow = "more>less" if mdiff > 0 else "less>more"
        lines.append(f"\n[{scale}] shapiro_normal={normal}  {test}  stat={stat}  "
                      f"{es}  mean_diff={mdiff:+.3f} ({arrow})")
        lines.append(f"    p_raw={p_raw:.4f}  p_holm={p_adj:.4f}  "
                      f"-> {'SIG' if p_adj < 0.05 else 'ns'}")
    return [(f"[{scale}] scenario {test}", p) for (scale, test, *_r), p in zip(results, pvals)]


# ---------- 5 : between-scenario, binary (McNemar) ----------
def between_scenario_binary(paired, lines):
    lines.append("\n" + "=" * 70)
    lines.append("STEP 5  BETWEEN-SCENARIO binary: McNemar per option")
    lines.append("=" * 70)
    blocks = {"expectation (Q4.4/Q5.4)": ("Q4.4", "Q5.4", 7),
              "verification (Q4.5/Q5.5)": ("Q4.5", "Q5.5", 8)}
    all_pvals = []   # [(label, p_raw)] across both blocks, for global correction
    for label, (a, b, nopt) in blocks.items():
        lines.append(f"\n-- {label} --")
        rows, pvals = [], []
        for i in range(1, nopt + 1):
            la, lb = f"{a}_{i}", f"{b}_{i}"
            sel_less = paired[la].notna().astype(int)
            sel_more = paired[lb].notna().astype(int)
            # 2x2 discordant cells
            b_cell = int(((sel_less == 1) & (sel_more == 0)).sum())  # less-only
            c_cell = int(((sel_less == 0) & (sel_more == 1)).sum())  # more-only
            tbl = [[int(((sel_less==0)&(sel_more==0)).sum()), c_cell],
                   [b_cell, int(((sel_less==1)&(sel_more==1)).sum())]]
            res = mcnemar(tbl, exact=(b_cell + c_cell) < 25)
            rows.append((i, b_cell, c_cell, res.pvalue)); pvals.append(res.pvalue)
            all_pvals.append((f"[scenario shift] {label} opt{i}", res.pvalue))
        holm = multipletests(pvals, method="holm")[1]
        for (i, bcell, ccell, _), padj in zip(rows, holm):
            direction = "more>less" if ccell > bcell else "less>more"
            flag = "SIG" if padj < 0.05 else "ns"
            lines.append(f"    opt{i}: less-only={bcell:3d} more-only={ccell:3d} "
                          f"({direction})  p_holm={padj:.4f}  {flag}")
    return all_pvals


# ---------- 6 : method switching ----------
def method_switching(paired, lines):
    lines.append("\n" + "=" * 70)
    lines.append("STEP 6  METHOD SWITCHING (Q4.2->Q5.2): Stuart-Maxwell + transitions")
    lines.append("=" * 70)
    d = paired[["Q4.2", "Q5.2"]].dropna()
    d = d[(d["Q4.2"] != "Other (Please specify)") & (d["Q5.2"] != "Other (Please specify)")]
    cats = sorted(set(d["Q4.2"]) | set(d["Q5.2"]))
    tab = pd.crosstab(d["Q4.2"], d["Q5.2"]).reindex(index=cats, columns=cats, fill_value=0)
    stayed = int(np.trace(tab.values)); total = int(tab.values.sum())
    lines.append(f"\nstayed on same method: {stayed}/{total} ({100*stayed/total:.0f}%)  "
                  f"switched: {total-stayed} ({100*(total-stayed)/total:.0f}%)")
    sm_pval = None
    try:
        sm = SquareTable(tab.values).homogeneity()
        sm_pval = sm.pvalue
        lines.append(f"Stuart-Maxwell marginal homogeneity: stat={sm.statistic:.3f}  "
                      f"df={sm.df}  p={sm.pvalue:.4f}  "
                      f"-> {'SIG (choices shift)' if sm.pvalue < 0.05 else 'ns'}")
    except Exception as e:
        lines.append(f"Stuart-Maxwell could not run: {e}")
    lines.append("\ntransition matrix (rows=less, cols=more), short labels:")
    short = {c: c.split(".")[0].split(" ")[0][:6] + "…" for c in cats}
    t2 = tab.rename(index=short, columns=short)
    lines.append(t2.to_string())
    return [("[method switching] Stuart-Maxwell", sm_pval)] if sm_pval is not None else []


if __name__ == "__main__":
    bases = get_bases(verbose=False)
    paired = bases["paired"]

    lines = [f"paired base n = {len(paired)}"]
    between_method(bases, lines)
    between_scenario_likert(paired, lines)
    between_scenario_binary(paired, lines)
    method_switching(paired, lines)

    for line in lines:
        print(line)

    report_path = save_report("stats", lines)
    print(f"\nreport saved -> {report_path}")
